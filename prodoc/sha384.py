...
def calculate_sha384_with_salt(password, salt):
    if salt:
        salted_password = salt + password
    else:
        salted_password = password
    return hashlib.sha384(salted_password.encode('utf-8')).hexdigest()

def check_sha384(wordlist_path, target_hash, salt):
    ...
    try:
        with open(wordlist_path, 'r', encoding='utf-8', errors='ignore') as file:
            for line in file:
                password = line.strip()
                sha384_hash = calculate_sha384_with_salt(password, salt)
                print(f"Trying: {password} -> {sha384_hash}")
                if sha384_hash == target_hash:
...
def brute_force_sha384(target_hash, salt, min_length, max_length, charset):
    start_time = datetime.now()
    try:
        for length in range(min_length, max_length + 1):
            for guess in itertools.product(charset, repeat=length):
                guess_str = ''.join(guess)
                sha384_hash = calculate_sha384_with_salt(guess_str, salt)
                print(f"Trying: {guess_str} -> {sha384_hash}")
                if sha384_hash == target_hash:
...
def main_sha384():
    print("SHA-384 Hash Cracker")
    print("1. Check SHA-384 hash against a wordlist")
    print("2. Brute force SHA-384 hash")
    option = input("Enter option (1 or 2): ")

    if option == '1':
        wordlist_path = input("Enter path to wordlist file (leave blank to use default rockyou.txt): ")
        target_hash = input("Enter SHA-384 hash to crack: ").lower()
        salt = input("Enter the salt value (leave blank for no salt): ")
        check_sha384(wordlist_path, target_hash, salt)
    elif option == '2':
        target_hash = input("Enter SHA-384 hash to crack: ").lower()
        salt = input("Enter the salt value (leave blank for no salt): ")
        ...
        brute_force_sha384(target_hash, salt, min_length, max_length, charset)
    else:
        print("Invalid option.")
